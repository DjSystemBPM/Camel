## JMS2 Component
*Available as of Camel version 2.20.0*
*Available as of Camel version 2.20*


### JMS2 Component

This component is an extension of the existing JMS component with support for using JMS 2.0 APIs.
This means you can use JMS 2.0 functionality such as a shared topic or shared durable topics.
...
--------------------------------------------
jms2:[queue:|topic:]destinationName[?options]
--------------------------------------------
...
The JMS2 component supports 75 options which are listed below.
...
The JMS2 endpoint is configured using URI syntax:

    jms2:destinationType:destinationName
...
[source,java]
----------------------------------------------------------------------------------------
from("file://inbox/order").to("jms2:queue:order?messageConverter=#myMessageConverter");
----------------------------------------------------------------------------------------
...
[source,java]
-----------------------------------------------------------------------
from("file://inbox/order").to("jms2:queue:order?jmsMessageType=Text");
-----------------------------------------------------------------------
...
[source,java]
---------------------------------------------------------------------------------------------------------
from("file://inbox/order").setHeader("CamelJmsMessageType", JmsMessageType.Text).to("jms2:queue:order");
---------------------------------------------------------------------------------------------------------
...
[source,java]
---------------------------------------------
from("jms2:SomeQueue?concurrentConsumers=20").
  bean(MyClass.class);
---------------------------------------------
...
[source,java]
----------------------------------------------------------------
from("jms2:SomeQueue?concurrentConsumers=20&asyncConsumer=true").
  bean(MyClass.class);
----------------------------------------------------------------
...
--------------------------------------------------------
   from("direct:someWhere")
     .to("jms2:queue:foo?replyTo=bar&requestTimeout=30s")
...
[source,java]
----------------------------------------------
from("jms2:topic:OrdersTopic").
  filter().method("myBean", "isGoldCustomer").
    to("jms2:queue:BigSpendersQueue");
----------------------------------------------
...
---------------------------------------------------
<route>
  <from uri="jms2:topic:OrdersTopic"/>
  <filter>
    <method bean="myBean" method="isGoldCustomer"/>
    <to uri="jms2:queue:BigSpendersQueue"/>
  </filter>
</route>
---------------------------------------------------
...
errorHandler(deadLetterChannel("jms2:queue:dead?transferExchange=true"));
------------------------------------------------------------------------
...
[source,java]
-----------------------------------------------------------------------------------
from("jms2:queue:dead").to("bean:myErrorAnalyzer");
...
from("seda:dead").transform(exceptionMessage()).to("jms2:queue:dead");
--------------------------------------------------------------------------------------------------
